...
enum { M_STAGES, M_PARAMS, M_EDIT, M_PRESET, M_IR, M_PGA, M_BACKING, M_BKLEVEL, M_LPLEVEL, M_BYPLEVEL, M_IRDRY };
...
static int s_sel   = 0;   // main rows: 0 back,1 Preset,2 IR,3 BK,4 BK level,5 LP level,6 BYP level,7 IR dry,8 Meters,9 I2C,10+ stage
...
#define N_SPECIAL 10      // rows before stages: back,Preset,IR,BK,BK level,LP level,BYP level,IR dry,Meters,I2C
...
bool menu_event(int turn, bool click) {
    ...
    if (s_mode == M_STAGES) {
        ...
        if (click) {
            ...
            else if (s_sel == 6) { s_mode = M_BYPLEVEL; }             // ⚠ Daisy addition: bypass make-up (A/B match)
            else if (s_sel == 7) { s_mode = M_IRDRY; }                // ⚠ Daisy addition: dry-blend on the IR (pick attack)
            else if (s_sel == 8) { app_gr_set(!app_gr_enabled()); }   // toggle ALL home meters in place
            else if (s_sel == 9) { app_i2c_cycle(); }                 // ⚠ Daisy addition: cycle bus clock
    ...
    if (s_mode == M_BKLEVEL) {                         // ⚠ Daisy addition: live backing level
        if (turn) {
    ...
    if (s_mode == M_BYPLEVEL) {                        // ⚠ Daisy addition: bypass make-up, live.
        // Trim it with the bypass switch under foot until engaged and bypassed match at PLAYING
        // level, then copy the number into the preset's byp.level -- it is not saved to the card.
        if (turn) dsp_chain_set_byp_level(dsp_chain_byp_level() + 0.05f * turn);
        if (click) s_mode = M_STAGES;
        return turn || click;
    }
    if (s_mode == M_IRDRY) {                           // ⚠ Daisy addition: dry-piezo blend on the IR, live.
        // Adds a high-passed (2.5 kHz) slice of the dry piezo back onto the wet IR so the pick-on-string
        // clack returns. Turn up until the attack is there without the quack; copy into the preset's
        // ir.dry -- not saved to the card. Only audible with an IR loaded.
        if (turn) app_ir_dry_set(app_ir_dry() + 0.05f * turn);
        if (click) s_mode = M_STAGES;
        return turn || click;
    }
    ...
    if (false) {                                       // (PGA mode removed on the Daisy port)
        if (turn)  app_pga_set_nib(app_pga_nib() + turn);
        if (click) s_mode = M_STAGES;
        return turn || click;
    }
...
void menu_render(void) {
    ...
    if (s_mode == M_STAGES) {
        ...
        for (int i = 0; i < VIS_ROWS && top + i < n_top; i++) {
            ...
            else if (it == 6) snprintf(line, sizeof line, "BYP level %.2f", (double)dsp_chain_byp_level());
            else if (it == 7) snprintf(line, sizeof line, "IR dry    %.2f", (double)app_ir_dry());
            else if (it == 8) snprintf(line, sizeof line, "Meters    %s", app_gr_enabled() ? "on" : "off");
            else if (it == 9) snprintf(line, sizeof line, "I2C     %4dk", app_i2c_khz());
            else {
        ...
        oled_text(0, 26, line);
        oled_text(0, 50, "turn to set, click ok");
    } else if (s_mode == M_BYPLEVEL) {
        oled_text(0, 0, "-- BYPASS LEVEL --");
        snprintf(line, sizeof line, "%.2f  (1.00 unity)", (double)dsp_chain_byp_level());
        oled_text(0, 26, line);
        oled_text(0, 42, "stomp bypass to A/B");
        oled_text(0, 50, "turn to set, click ok");
    } else if (s_mode == M_IRDRY) {
        oled_text(0, 0, "-- IR DRY BLEND --");
        snprintf(line, sizeof line, "%.2f  (0 = pure IR)", (double)app_ir_dry());
        oled_text(0, 26, line);
        oled_text(0, 42, "adds pick attack");
        oled_text(0, 50, "turn to set, click ok");
