...
#include "ff.h"
// ⚠ Two platform calls only -- the streaming engine itself is portable. The Pico's I2S rate
// constant and its microsecond timer come from the shim; nothing else here is RP2350-specific,
// so this file ports byte-for-byte apart from this include.
...
static FIL      s_fil;
// ⚠ The 64 KB ring lives in SDRAM, not DTCM. On the RP2350 it fitted in main RAM; here it
// overflowed DTCMRAM by 976 bytes and refused to link. The Daisy has 64 MB of SDRAM that nothing
// else uses, and a streamed audio ring is exactly what it is for -- sequential access, refilled in
// the foreground, never on a latency-critical path.
static int16_t DSY_SDRAM_BSS s_ring[RING_SAMPLES];
